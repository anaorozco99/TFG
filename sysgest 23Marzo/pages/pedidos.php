...
if (isset($_GET['recibir']) && (int)$_GET['recibir'] > 0) {
    ...
    if ($pedido && $pedido['estado'] !== 'recibido') {
        $conn->prepare("UPDATE pedidos SET estado = 'recibido' WHERE id = ?")
             ->bind_param('i', $pid);
        $conn->prepare("UPDATE pedidos SET estado = 'recibido' WHERE id = ?")
             ->execute();
        ...
        <main class="main">
            ...
            <div class="panel">
                <table>
                    <thead>
                        <tr>
                            <th>#</th><th>Solicitado por</th><th>Fecha pedido</th>
                            <th>Llegada estimada</th><th>Artículos</th><th>Estado</th><th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php if ($pedidos && $pedidos->num_rows > 0): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="#" onclick="abrirModal('det-<?= $p['id'] ?>')"
                            ...
                            <td>
                                <?php if ($p['estado'] !== 'recibido' && $llegado): ?>
                                    <a href="?recibir=<?= $p['id'] ?>&filtro=<?= $filtro ?>"
                                       class="btn btn-verde btn-sm"
                                       onclick="return confirm('¿Confirmar recepción? Se actualizará el stock.')">
                                        ✅ Marcar recibido
                                    </a>
                                <?php elseif ($p['estado'] !== 'recibido'): ?>
                                    <span class="text-gris" style="font-size:12px;">Esperando llegada</span>
                                <?php else: ?>
                        ...
                        <div class="modal-overlay" id="det-<?= $p['id'] ?>">
                            <div class="modal">
                                ...
                                <div class="modal-body">
                                    <?php
                                    $lineas = $conn->prepare("SELECT nombre, cantidad FROM pedidos_lineas WHERE pedido_id = ?");
                                    $lineas->bind_param('i', $p['id']);
                                    $lineas->execute();
                                    $lres = $lineas->get_result();
                                    ?>
                                    <table>
...
<script>
// Refresca cada 5 minutos para actualizar las cuentas atrás
