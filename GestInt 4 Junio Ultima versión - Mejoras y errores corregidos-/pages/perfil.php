<?php
// Perfil del usuario: datos personales, fichar entrada/salida y subir justificantes
        ...
        <div style="display:grid;grid-template-columns:auto 1fr;gap:20px;align-items:center;">
          <form method="POST">
            ...
            <?php elseif ($puede_entrada): ?>
              <!-- Pide confirmación antes de registrar la entrada -->
              <button type="submit" class="btn btn-verde" style="font-size:15px;padding:12px 28px;"
                onclick="return confirm('¿Confirmas que quieres registrar tu ENTRADA ahora?')">
                ✅ Registrar entrada
              </button>
            <?php else: ?>
              <!-- Pide confirmación antes de registrar la salida -->
              <button type="submit" class="btn btn-rojo" style="font-size:15px;padding:12px 28px;"
                onclick="return confirm('¿Confirmas que quieres registrar tu SALIDA ahora?')">
                🚪 Registrar salida
              </button>
            <?php endif; ?>
          </form>
    ...
    <div class="panel">
      ...
      <table>
        ...
        <tr>
          ...
          <td><?= htmlspecialchars($j['descripcion']) ?></td>
          <td><a href="../uploads/justificantes/<?= $j['archivo'] ?>" target="_blank" class="btn btn-gris btn-sm">📄 Ver</a></td>
          <td class="text-gris"><?= date('d/m/Y H:i', strtotime($j['fecha'])) ?></t