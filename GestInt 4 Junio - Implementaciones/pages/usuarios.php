...
// el rol soporte IT es el admin de sistema (no se puede desactivar)
function esAdminSistema($conn, $id): bool {
    $s = $conn->prepare("SELECT rol FROM usuarios WHERE id = ?");
    $s->bind_param('i', $id);
    $s->execute();
    $r = $s->get_result()->fetch_assoc();
    return $r && $r['rol'] === 'soporte';
}
...
if (in_array($accion, ['activar','desactivar']) && $id > 0) {
    if ($id === (int)$_SESSION['usuario_id']) {
        ...
        $msg = 'El administrador del sistema no puede ser desactivado.'; $tipo = 'error';
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($edit_id > 0 && esAdminSistema($conn, $edit_id)) {
        $datos['rol'] = 'admin'; $datos['usuario'] = 'admin';
    }
            ...
            <div class="panel">
                <table>
                    ...
                    <?php while ($u = $usuarios->fetch_assoc()):
                        $es_admin_sistema = $u['rol'] === 'soporte';
                    ...
                    <tr>
                        <td class="fw-600">
                            ...
                            <?php if ($es_admin_sistema): ?><span class="badge badge-rojo" style="margin-left:4px;font-size:10px;">Sistema</span><?php endif; ?>
                        </td>
...
<div class="modal-overlay <?= in_array($accion,['nuevo','editar'])?'abierto':'' ?>" id="modal-usuario">
    <div class="modal">
        ...
        <form method="POST">
            ...
            <?php $editando_admin_sistema = $usr_editar && $usr_editar['rol'] === 'soporte'; ?>
            <div class="modal-body">
                <?php if ($editando_admin_sistema): ?>
                    <div class="alerta alerta-aviso">Este es el administrador del sistema. Solo se puede cambiar su contraseña.</div>
                <?php endif; ?>
                <div class="form-grid">
                    <?php if (!$editando_admin_sistema): ?>
                    ...
                    <div class="form-grupo">
                        <label>Rol</label>
                        <select name="rol">
                            <?php foreach (['admin'=>'Administrador','soporte'=>'Soporte IT','rrhh'=>'Resp. RRHH','almacen'=>'Resp. Almacén','empleado'=>'Empleado'] as $k=>$v): ?>
