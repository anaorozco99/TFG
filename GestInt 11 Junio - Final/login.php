<?php
// pantalla de login y formulario de ticket sin autenticación
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion_login'])) {
    ...
    if ($usuario === '' || $contra === '') {
        ...
        // buscamos el usuario activo y verificamos la contraseña
        ...
        if ($fila && password_verify($contra, $fila['password'])) {
            // guardamos en sesión y registramos el acceso
...
// ticket abierto desde la pantalla de login (sin estar logueado)
    ...
    <?php else: ?>
      ...
      <form method="POST" novalidate>
        ...
        <div class="form-grupo">
          ...
          <input type="password" id="contra" name="contra" autocomplete="current-password" required>
        </div>
