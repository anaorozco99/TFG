...
if (!puedeClientes()) { header('Location: dashboard.php'); exit; }
...
// desactivar cliente (no se borra, solo se oculta)
$puede_editar_clientes = puedeEditarClientes();

if ($accion === 'desactivar' && $id > 0 && $puede_editar_clientes) {
...
// teléfono: vacío (opcional) o 9 dígitos españoles con prefijo +34 opcional
function tel_valido_cli(string $tel): bool {
    if ($tel === '') return true;
    return (bool)preg_match('/^(\+34[\s\-]?)?[6789]\d{8}$/', preg_replace('/[\s\-]/','',$tel));
}

// guardar cliente nuevo o editado
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $puede_editar_clientes) {
    ...
    if (!tel_valido_cli($datos['telefono']))                     $errores[] = 'Teléfono no válido (ej: 612345678 o +34 612345678).';
...
$cli_editar = null;
if ($accion === 'editar' && $id > 0 && $puede_editar_clientes) {
