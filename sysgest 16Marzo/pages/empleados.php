<?php
// ============================================================
// GestInt — Gestión de Empleados (CRUD completo)
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!preg_match('/^\d{8}[A-Z]$/', strtoupper($datos['dni']))) $errores[] = 'DNI inválido (8 dígitos + letra).';
    ...
    if (empty($errores)) {
        ...
        if ($edit_id > 0) {
            ...
            $stmt->execute();
            $msg  = 'Empleado dado de alta correctamente.';
        }
        $tipo   = 'ok';
        $accion = ''; // cierra modal
    } else {
        $msg   = implode(' ', $errores);
        $tipo  = 'error';
        $accion = 'nuevo'; // mantiene modal abierto
    }
}
            ...
            <div class="panel" style="margin-bottom:16px;">
                <div class="panel-body" style="padding:12px 16px;">
                    <form method="GET" class="barra-filtros">
                        <input type="text" name="buscar" placeholder="Buscar por nombre, DNI, email..."
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-8">
                                    <a href="?accion=editar&id=<?= $e['id'] ?>"
                                       class="btn btn-gris btn-sm">✏️ Editar</a>
                                    <a href="?accion=eliminar&id=<?= $e['id'] ?>"
                                       class="btn btn-rojo btn-sm"
                                       onclick="return confirm('¿Dar de baja a este empleado?')">Baja</a>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="8" class="text-gris text-center" style="padding:24px;">
...
<div class="modal-overlay <?= ($accion === 'nuevo' || $accion === 'editar') ? 'abierto' : '' ?>"
     id="modal-empleado">
    <div class="modal">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-grupo">
                        ...
                        <select name="departamento">
                            <?php foreach (['Dirección','RRHH','Informática','Administración','Logística','Almacén','Ventas','Producción'] as $dep): ?>
