...
// contabilidad y direccion pueden ver pero no editar
if (!puedeAlmacen() && $_SESSION['rol'] !== 'contabilidad') { header('Location: dashboard.php'); exit; }

$solo_lectura = !puedeEditarPedidosStock();

$msg  = '';
$tipo = '';

// marcar pedido como recibido y sumar las unidades al inventario
if (isset($_GET['recibir']) && (int)$_GET['recibir'] > 0) {
    ...
    if ($pedido && $pedido['estado'] !== 'recibido') {
        ...
        // actualizamos el stock de cada artículo del pedido
...
// filtramos por estado si viene en el get
            ...
            <div class="panel">
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($p['estado']!=='recibido' && !$solo_lectura): ?>
                                <a href="#"
