<?php
// pedidos de ventas a clientes: listado, cambio de estado y cancelación
require_once '../includes/auth.php';
require_once '../includes/db.php';

if (!puedePedidosClientes()) { header('Location: dashboard.php'); exit; }

$msg  = '';
$tipo = '';

// ventas y almacen pueden cambiar estado y cancelar; contabilidad/recepcion/direccion solo leen
$puede_cambiar_estado = puedeEditarPedidosClientes();
// contabilidad solo ve factura en pedidos enviados o entregados
$es_contabilidad = $_SESSION['rol'] === 'contabilidad';

// procesamos el cambio de estado si viene por get
if (isset($_GET['accion']) && isset($_GET['id']) && $puede_cambiar_estado) {
    $pid   = (int)$_GET['id'];
    $nueva = $_GET['accion'];

    // solo aceptamos estos tres estados
    if (in_array($nueva, ['enviado', 'entregado', 'cancelado']) && $pid > 0) {

        // no se puede cancelar si ya está enviado o entregado
        if ($nueva === 'cancelado') {
            ...
            if ($estado_actual === 'enviado' || $estado_actual === 'entregado') {
                ...
                $tipo = 'error';
                $nueva = ''; // vaciamos para no hacer el update
            }
        }
...
// cogemos los filtros activos del get
$filtro          = $_GET['filtro']          ?? 'todos';
$filtro_cliente  = (int)($_GET['cliente']   ?? 0);
$filtro_vendedor = (int)($_GET['vendedor']  ?? 0);

// montamos el where según lo que haya seleccionado el usuario
$where = "WHERE 1=1";
...
<div class="contenido">
    <div class="topbar">
        ...
        <div class="topbar-right">
            <?php if (puedeCrearPedidosClientes()): ?>
