<?php
// Reponer stock: seleccionar artículos y generar un pedido de reposición al proveedor
require_once '../includes/auth.php';
require_once '../includes/db.php';

if (!puedeEditarPedidosStock()) { header('Location: dashboard.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'añadir') {
        ...
        if (empty($_SESSION['carrito'])) {
            ...
            $lineas_pedido = $_SESSION['carrito']; // guardar antes de vaciar
            ...
            // ── N8N: notificar pedido de reposición de stock ─────────────────
            require_once '../includes/n8n.php';
            $articulos_pedido = array_map(fn($l) => $l['nombre'] . ' (x' . $l['cantidad'] . ')', $lineas_pedido);
            n8n_notify(N8N_WEBHOOK_PEDIDO_STOCK, [
                'pedido_id' => $pedido_id,
                'usuario'   => $_SESSION['nombre'] ?? '',
                'articulos' => implode(', ', $articulos_pedido),
                'total_lineas' => count($lineas_pedido),
