...
$tab_map = ['calendario'=>'tab_cal','solicitudes'=>'tab_sol','aprobadas'=>'tab_aprobadas','gestion'=>'gestion_sol'];
$paginaActiva = $tab_map[$tab] ?? 'tab_cal';

// RRHH: aprobar, rechazar o eliminar vacación aprobada
if (puedeAprobarVacaciones() && isset($_GET['accion']) && isset($_GET['id'])) {
    $vid    = (int)$_GET['id'];
    $accion = $_GET['accion'];
    if (in_array($accion, ['aprobar','rechazar'])) {
        ...
        log_act($accion, 'vacaciones', $vid, "Estado cambiado a $estado");
    } elseif ($accion === 'eliminar') {
        $del = $conn->prepare("DELETE FROM vacaciones WHERE id=? AND estado='aprobada'");
        $del->bind_param('i', $vid);
        $del->execute();
        log_act('eliminar', 'vacaciones', $vid, "Vacación aprobada eliminada");
        $msg = 'Vacación eliminada.'; $tipo = 'aviso';
        $tab = 'aprobadas';
    }
}

// RRHH/admin: editar vacación aprobada
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'editar_vacacion' && puedeAprobarVacaciones()) {
    $vid = (int)$_POST['id'];
    $fi  = $_POST['fecha_ini'] ?? '';
    $ff  = $_POST['fecha_fin'] ?? '';
    ...
    if ($fi && $ff && $ff >= $fi) {
        ...
        $upd  = $conn->prepare("UPDATE vacaciones SET fecha_ini=?, fecha_fin=?, dias=?, motivo=? WHERE id=? AND estado='aprobada'");
        $upd->bind_param('ssisi', $fi, $ff, $dias, $mot, $vid);
        $upd->execute();
        log_act('editar', 'vacaciones', $vid, "Fechas: $fi → $ff ($dias días)");
        $msg = 'Vacación actualizada.'; $tipo = 'ok';
    } else {
        $msg = 'Fechas no válidas.'; $tipo = 'error';
    }
    $tab = 'aprobadas';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'solicitar') {
    ...
    if (!$fi || !$ff) { $msg = 'Indica las fechas.'; $tipo = 'error'; }
        ...
        // N devuelve 1=lunes ... 7=domingo; >= 6 significa sábado o domingo
        $dow_ini = (int)(new DateTime($fi))->format('N');
        $dow_fin = (int)(new DateTime($ff))->format('N');
        if ($dow_ini >= 6) { $msg = 'La fecha de inicio cae en fin de semana (' . (new DateTime($fi))->format('l d/m/Y') . '). Las vacaciones deben empezar en día laborable (lunes a viernes).'; $tipo = 'error'; }
        elseif ($dow_fin >= 6) { $msg = 'La fecha de fin cae en fin de semana (' . (new DateTime($ff))->format('l d/m/Y') . '). Las vacaciones deben terminar en día laborable (lunes a viernes).'; $tipo = 'error'; }
        else {
        $dias = (int)(new DateTime($fi))->diff(new DateTime($ff))->days + 1;
...
// vacaciones aprobadas para el calendario: rrhh ve todo, el resto solo su departamento (o solo las suyas si no está en ningún depto)
$vac_where = puedeAprobarVacaciones()
...
// expandimos cada rango de fechas a días sueltos para el calendario
$dias_cal = [];
foreach ($vac_cal as $v) {
...
// Vacaciones aprobadas (lista) para RRHH y dirección (solo lectura)
$aprobadas = [];
if (puedeAprobarVacaciones() || esDireccion()) {
...
<div class="contenido">
    <div class="topbar">
        <h1>Vacaciones</h1>
        <div class="topbar-right text-gris"><?php if ($_SESSION['rol'] === 'it'): ?>Vista exclusiva RRHH<?php endif; ?></div>
    </div>
    <main class="main">
        ...
        <div class="panel" style="margin-bottom:16px;">
            <div class="panel-body" style="padding:8px 16px;">
                <div class="barra-filtros">
                    ...
                    <?php if (puedeAprobarVacaciones() || esDireccion()): ?>
                    ...
                    <a href="vacaciones.php?tab=gestion" class="btn <?= $tab==='gestion' ?'btn-primario':'btn-gris' ?> btn-sm">⚙ Gestión de solicitudes</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
                ...
                <div class="panel"><div class="panel-header"><h2>Leyenda</h2></div>
                    ...
                    <?php $shown=[];foreach($vac_cal as $v):if(in_array($v['uid'],$shown))continue;$shown[]=$v['uid']; ?>
                        <div class="leyenda-item">
                            ...
                            <?php if(puedeAprobarVacaciones()&&$v['departamento']): ?><span class="badge badge-azul" style="font-size:10px;"><?=htmlspecialchars($v['departamento'])?></span><?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    <form method="POST" class="form-grid">
                        ...
                        <div class="form-grupo full"><label>Fecha inicio *</label><input type="date" name="fecha_ini" required min="<?=date('Y-m-d')?>">
                            <p style="font-size:11px;color:#d97706;margin-top:4px;">Solo días laborables (lun–vie)</p></div>
                        <div class="form-grupo full"><label>Fecha fin *</label><input type="date" name="fecha_fin" required min="<?=date('Y-m-d')?>">
                            <p style="font-size:11px;color:#d97706;margin-top:4px;">Solo días laborables (lun–vie)</p></div>
        ...
        <!-- ===== TAB: APROBADAS (RRHH) ===== -->
        <?php elseif ($tab === 'aprobadas' && (puedeAprobarVacaciones() || esDireccion())): ?>
        <div class="panel">
            <div class="panel-header"><h2>✅ Vacaciones aprobadas</h2></div>
            <?php if (!empty($aprobadas)): ?>
            <table>
                <thead><tr><th>Empleado</th><th>Departamento</th><th>Período</th><th>Días</th><th>Motivo</th><?php if(puedeAprobarVacaciones()): ?><th>Acciones</th><?php endif; ?></tr></thead>
                <tbody>
                    ...
                    <td>
                        <div class="d-flex gap-8">
                            <button class="btn btn-gris btn-sm"
                                onclick="abrirEditarVacacion(<?=$v['id']?>,'<?=htmlspecialchars($v['fecha_ini'],ENT_QUOTES)?>','<?=htmlspecialchars($v['fecha_fin'],ENT_QUOTES)?>','<?=htmlspecialchars($v['motivo']??'',ENT_QUOTES)?>')">
                                Editar
                            </button>
                            <a href="?tab=aprobadas&accion=eliminar&id=<?=$v['id']?>"
                               class="btn btn-rojo btn-sm"
                               onclick="return confirm('¿Eliminar esta vacación aprobada?')">Eliminar</a>
                        </div>
                    </td>
                    <?php endif; ?>
                </tr>
        ...
        <!-- Modal editar vacación aprobada -->
        <?php if(puedeAprobarVacaciones()): ?>
        <div class="modal-overlay" id="modal-editar-vac">
            <div class="modal" style="max-width:440px;">
                <div class="modal-header">
                    <h3>Editar vacación aprobada</h3>
                    <button class="modal-cerrar" onclick="cerrarModal('modal-editar-vac')">✕</button>
                </div>
                <form method="POST">
                    <input type="hidden" name="accion" value="editar_vacacion">
                    <input type="hidden" name="id" id="edit-vac-id">
                    <div class="modal-body">
                        <div class="form-grupo">
                            <label>Fecha inicio *</label>
                            <input type="date" name="fecha_ini" id="edit-vac-ini" required>
                        </div>
                        <div class="form-grupo">
                            <label>Fecha fin *</label>
                            <input type="date" name="fecha_fin" id="edit-vac-fin" required>
                        </div>
                        <div class="form-grupo">
                            <label>Motivo</label>
                            <input type="text" name="motivo" id="edit-vac-motivo">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-gris" onclick="cerrarModal('modal-editar-vac')">Cancelar</button>
        ...
        <script>
        function abrirEditarVacacion(id, ini, fin, motivo) {
            document.getElementById('edit-vac-id').value    = id;
            document.getElementById('edit-vac-ini').value   = ini;
            document.getElementById('edit-vac-fin').value   = fin;
            document.getElementById('edit-vac-motivo').value = motivo;
            abrirModal('modal-editar-vac');
        }
        </script>
        <?php endif; ?>

        <!-- ===== TAB: GESTIÓN RRHH ===== -->
        <?php elseif ($tab === 'gestion' && puedeAprobarVacaciones()): ?>
        <div class="panel">
            <div class="panel-header">
...
<script>
// Validación fin de semana en inputs de fecha de vacaciones
document.querySelectorAll('input[type="date"][name="fecha_ini"], input[type="date"][name="fecha_fin"]').forEach(function(input) {
    input.addEventListener('change', function() {
        if (!this.value) return;
        var d = new Date(this.value + 'T12:00:00');
        var dow = d.getDay(); // 0=domingo, 6=sábado
        if (dow === 0 || dow === 6) {
            alert('No se pueden solicitar vacaciones en fin de semana (sábado o domingo).');
            this.value = '';
        }
    });
});
