...
// Usuarios + empleados vinculados (se crean juntos)
$datos = [
    ['Administrador', 'Sistema',       'admin',  'admin@gestint.local',  'Admin1234',  'admin',   null],
    ['Pedro',         'Martínez Ruiz', 'pedro',  'pedro@gestint.local',  'Pedro1234',  'admin',   ['22222222B','pedro@gestint.local','600333444','Informática','Sysadmin','2021-06-15',32000]],
    ['María',         'García López',  'maria',  'maria@gestint.local',  'Maria1234',  'rrhh',    ['11111111A','maria@gestint.local','600111222','RRHH','Técnico RRHH','2022-03-01',28000]],
    ['Carlos',        'Fernández Gil', 'carlos', 'carlos@gestint.local', 'Carlos1234', 'almacen', ['44444444D','carlos@gestint.local','600777888','Almacén','Resp. Almacén','2020-09-01',26000]],
    ['Laura',         'Sánchez Mora',  'laura',  'laura@gestint.local',  'Laura1234',  'empleado',['33333333C','laura@gestint.local','600555666','Administración','Aux. Administración','2023-01-10',22000]],
    ['Ana',           'Jiménez Vega',  'ana',    'ana@gestint.local',    'Ana12345',   'empleado',['55555555E','ana@gestint.local','600999000','Ventas','Comercial','2023-05-20',24000]],
];
...
foreach ($datos as [$nom, $ape, $usr, $email, $pass, $rol, $emp]) {
    ...
    // Si tiene datos de empleado, lo vinculamos
    if ($emp) {
        // Obtener el id del usuario recién creado
        $res = $conn->query("SELECT id FROM usuarios WHERE usuario = '$usr' LIMIT 1");
        $uid = $res->fetch_row()[0];

        [$dni, $emp_email, $tel, $dep, $cargo, $fecha, $sal] = $emp;

        $stmtE = $conn->prepare(
            "INSERT IGNORE INTO empleados (id_usuario, nombre, apellidos, dni, email, telefono, departamento, cargo, fecha_alta, salario)
             VALUES (?,?,?,?,?,?,?,?,?,?)"
        );
        ...
        if ($conn->error) {
            ...
            $ok[] = "Empleado <b>$nom $ape</b> vinculado al usuario <b>$usr</b>";
        }
    }
}
...
<head>
    ...
    <style>
        ...
        h1    { color: #2C4A50; margin-bottom: 24px; }
        ...
        .btn  { display:inline-block; margin-top:24px; padding:12px 28px; background:#587A80; color:#fff; border-radius:6px; text-decoration:none; font-weight:600; }
        ...
        th    { background:#2C4A50; color:#fff; padding:8px 12px; text-align:left; }
...
<body>
    ...
    <?php if (empty($err)): ?>
        ...
        <h2 style="margin-top:28px;color:#2C4A50;">Usuarios creados</h2>
        <table>
            <tr><th>Usuario</th><th>Contraseña</th><th>Rol</th><th>Empleado vinculado</th></tr>
            <tr><td>admin</td> <td>Admin1234</td>  <td>Administrador</td><td>—</td></tr>
            <tr><td>pedro</td> <td>Pedro1234</td>  <td>Administrador</td><td>Pedro Martínez</td></tr>
            <tr><td>maria</td> <td>Maria1234</td>  <td>Resp. RRHH</td>  <td>María García</td></tr>
            <tr><td>carlos</td><td>Carlos1234</td> <td>Resp. Almacén</td><td>Carlos Fernández</td></tr>
            <tr><td>laura</td> <td>Laura1234</td>  <td>Empleado</td>    <td>Laura Sánchez</td></tr>
            <tr><td>ana</td>   <td>Ana12345</td>   <td>Empleado</td>    <td>Ana Jiménez</td></tr>
        </table>
