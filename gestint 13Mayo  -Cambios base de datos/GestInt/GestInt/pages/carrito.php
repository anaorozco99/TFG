    ...
    if ($accion === 'añadir') {
        ...
        if ($inv_id > 0) {
            ...
            if ($art) {
                if (isset($_SESSION['carrito'][$inv_id])) {
                    ...
                    $_SESSION['carrito'][$inv_id] = [
                        'inventario_id' => $inv_id,
                        'nombre'        => $art['nombre'],
                        'cantidad'      => $cantidad,
                    ];
                }
                $msg  = '"' . htmlspecialchars($art['nombre']) . '" añadido al carrito.';
                $tipo = 'ok';
            }
        }
        ...
        if (empty($_SESSION['carrito'])) {
            ...
            foreach ($_SESSION['carrito'] as $linea) {
                $stmtL->bind_param('iisi',
                    $pedido_id, $linea['inventario_id'], $linea['nombre'], $linea['cantidad']
                );
                $stmtL->execute();
            }
...
$articulos = $conn->query(
    "SELECT id, nombre, cantidad, stock_minimo, unidad FROM inventario ORDER BY nombre"
);
...
<head>
    ...
    <title>GestInt — Pedir stock</title>
                    ...
                    <div class="panel-header">
                        ...
                        <?php if (!empty($_SESSION['carrito'])): ?>
                            <form method="POST" style="margin:0;">
                                ...
                                <button type="submit" class="btn btn-gris btn-sm"
                                        onclick="return confirm('¿Vaciar el carrito?')">🗑 Vaciar</button>
                            </form>
                        <?php endif; ?>
                    </div>

                    <?php if (empty($_SESSION['carrito'])): ?>
                        <div class="panel-body text-gris text-center" style="padding:30px;">
                            El carrito está vacío.
                        </div>
                    <?php else: ?>
                        <table>
                            <thead>
                                <tr><th>Artículo</th><th style="width:120px;">Cantidad</th><th style="width:50px;"></th></tr>
                            </thead>
                            <tbody>
                        ...
                        <div class="panel-body" style="border-top:1px solid var(--gris-borde);">
                            <form method="POST" class="form-grid">
                                ...
                                <div class="form-grupo full">
                                    ...
                                    <button type="submit" class="btn btn-verde w-100" style="padding:14px;font-size:15px;"
