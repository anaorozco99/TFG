...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($edit_id > 0 && esProtegido($conn, $edit_id) && !$es_admin_puro) {
        ...
        // Validar que el rol enviado es uno permitido para el usuario actual (evita manipulación del POST)
        $roles_permitidos = $es_admin_puro
            ? ['admin','director','soporte','rrhh','almacen','empleado']
            : ['rrhh','almacen','empleado'];
...
$filtro_rol = $_GET['rol'] ?? '';
$orden_roles = "FIELD(rol,'admin','director','soporte','rrhh','almacen','empleado')";
$where_u = "WHERE 1=1";
if ($buscar_u !== '') {
                ...
                <div class="panel-body" style="padding:10px 16px;">
                    <form method="GET" class="barra-filtros">
                        ...
                        <select name="rol" onchange="this.form.submit()">
                            ...
                            <?php foreach(['admin'=>'Administrador','director'=>'Director','soporte'=>'Soporte IT','rrhh'=>'Resp. RRHH','almacen'=>'Resp. Almacén','empleado'=>'Ventas'] as $k=>$v): ?>
