...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Campos extra solo al crear (no al editar)
    if ($edit_id === 0) {
    ...
    if (empty($errores)) {
        ...
        if ($edit_id > 0) {
            ...
            // Crear usuario del sistema primero
            $roles_validos = ['admin','rrhh','almacen','empleado'];
            $rol_u = in_array($datos['rol'], $roles_validos) ? $datos['rol'] : 'empleado';
            $hash  = password_hash($datos['contra'], PASSWORD_DEFAULT);
            $su = $conn->prepare(
                "INSERT INTO usuarios (nombre,apellidos,usuario,email,rol,password,activo) VALUES (?,?,?,?,?,?,1)"
            );
            $su->bind_param('ssssss',
                $datos['nombre'], $datos['apellidos'], $datos['usuario'],
                $datos['email'],  $rol_u, $hash
            );
            $su->execute();
            $uid = $conn->insert_id;

            // Crear empleado vinculado al usuario recién creado
            $stmt = $conn->prepare(
            ...
            <div class="modal-body">
                <div class="form-grid">
                    ...
                    <div class="form-grupo">
                        <label>Rol</label>
                        <select name="rol">
                            <?php foreach (['empleado'=>'Empleado','rrhh'=>'Resp. RRHH','almacen'=>'Resp. Almacén','admin'=>'Administrador'] as $k=>$v): ?>
