...
if (!esContabilidad()) { header('Location: dashboard.php'); exit; }
...
// ---- Distribución por cliente (top ventas) ----
$dist_clientes = $conn->query("
    SELECT c.nombre, IFNULL(SUM(pv.total),0) AS total
    FROM pedidos_ventas pv JOIN clientes c ON c.id = pv.cliente_id
    WHERE pv.estado != 'cancelado'
...
.graficos-grid { display:grid; grid-template-columns:2fr 1fr; gap:16px; margin-bottom:16px; }
...
<div class="contenido">
    ...
    <main class="main">
        ...
        <div class="graficos-grid">
            ...
            <div class="chart-wrap">
                <div class="chart-titulo">Distribución ventas por cliente</div>
...
// ---- Gráfico distribución clientes (pie) ----
<?php
$pie_labels  = array_column($dist_clientes, 'nombre');
$pie_valores = array_map(fn($r) => round((float)$r['total'],2), $dist_clientes);
